...
int processArguments(int argc, const char * argv[] ){
    ...
    do {} 
    while ( !loc.goodLocationFound && [[NSRunLoop currentRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate distantFuture]] );
    [loc release];
...
//    printf( "  -v          Verbose mode\n");
    ...
    printf( "\n ");
    printf( "Command: %s -f \"<lat>{LAT}</lat><lon>{LON}</lon><alt>{ALT}</alt>\"\n", [[[NSString stringWithUTF8String:argv[0]] lastPathComponent] UTF8String] );
...
-(NSString *)urlencode:(NSString *)raw{
    ...
    NSString * encodedString = [raw stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
